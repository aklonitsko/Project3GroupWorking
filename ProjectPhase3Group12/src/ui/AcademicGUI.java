...
implements ActionListener,TableModelListener, Observer {
	...
	private JPanel createAcaPnl() {
		...
		if(mStudent.getAcademicRecord() == null){
			AcademicRecord nmAC;
			String id = mStudent.getID();
			nmAC = new AcademicRecord(id, "Na", "Na", 
					"Na", "Na", "Na", "Na", 0.0, new ArrayList<TransferSchool>());
			mStudent.addAcademicRecord(nmAC);
		}
	...
	private void performAddTransfer() {
		...
		if (mStudent.getAcademicRecord().addTransferSchool(mTF)) {
	...
	private void performAddAcad() {
		...
		if(mStudent.getAcademicRecord().getProgram() == null)
		{
			...
			if(checkFields == 0){
				mStudent.getAcademicRecord().setProgram(txfField[0].getText());
				mStudent.getAcademicRecord().setGraduationTerm(txfField[1].getText());
				mStudent.getAcademicRecord().setGraduationTerm(txfField[2].getText());
				mStudent.getAcademicRecord().setGraduationYear(txfField[3].getText());
				mStudent.getAcademicRecord().setUWEmail(txfField[4].getText());
				mStudent.getAcademicRecord().setExternalEmail(txfField[5].getText());
				mStudent.getAcademicRecord().setGPA(Double.parseDouble(txfField[6].getText()));
